...
test.describe('PSP Dashboard Screenshots', () => {
  ...
  test('capture sessions page', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: path.join(screenshotsDir, 'create-session.png')
    });
    await page.click('text=Cancel');
  });

  test('capture session details page', async ({ page }) => {
    ...
    await page.screenshot({ 
      path: path.join(screenshotsDir, 'session-details.png'),
      fullPage: true 
    });
    ...
    for (const tab of tabs) {
      ...
      await page.screenshot({ 
        path: path.join(screenshotsDir, `session-details-${cleanFileName(tab)}.png`),
        fullPage: false 
      });
    }
  });

  test('capture recorder page', async ({ page }) => {
    await page.goto('http://localhost:3000/recorder');
    // Wait for content to load
    await page.waitForSelector('text=Session Recorder', { state: 'visible' });
    await page.waitForTimeout(1000); // Wait for animations
    await page.screenshot({ 
      path: path.join(screenshotsDir, 'recorder.png'),
      fullPage: true 
    });
  });
