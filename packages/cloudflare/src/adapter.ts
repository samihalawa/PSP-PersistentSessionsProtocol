...
export class CloudflareAdapter extends Adapter {
  ...
  private options: CloudflareAdapterOptions;
  ...
  constructor(options: CloudflareAdapterOptions = {}) {
    ...
    if (storageProvider && storageProvider.type === 'cloudflare-kv') {
      this.setStorageProvider(new CloudflareKVStorageProvider(
        storageProvider.namespace,
        storageProvider.keyPrefix
      ));
    }
   ...
   */
  async connect(renderer: any): Promise<void> {
 ...
 */
class CloudflareKVStorageProvider implements StorageProvider {
   ...
   */
  constructor(namespace: KVNamespace, keyPrefix = 'psp_session_') {
  ...
  async list(filter?: any): Promise<any[]> {
    ...
    const metadataPromises = keys.map(key => 
      this.namespace.get(key.name, 'json')
    );
    ...
    if (filter) {
      ...
      if (filter.tags && filter.tags.length > 0) {
        metadata = metadata.filter(m => 
          filter.tags.every((tag: string) => m.tags?.includes(tag))
        );
      }
   ...
   */
  private getExpirationSeconds(expireAt?: number): number | undefined {
    if (!expireAt) {
      return undefined;
    }
